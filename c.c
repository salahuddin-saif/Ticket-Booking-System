/*
 * ======================================================================
 *                 TICKET BOOKING SYSTEM WITH ROUTE MANAGEMENT
 * ======================================================================
 * PROJECT: Online Ticket Booking System (Enhanced with Graphs, Trees, Priority Queues)
 * ======================================================================
 */
...
/* ======================================================================
 * DATA STRUCTURE 1: PRIORITY QUEUE FOR SEAT ALLOCATION
 * ====================================================================== */
...
/* ======================================================================
 * DATA STRUCTURE 2: GRAPH FOR ROUTE MANAGEMENT
 * ====================================================================== */
...
/* ======================================================================
 * DATA STRUCTURE 3: TREE FOR SEAT AVAILABILITY (BST)
 * ====================================================================== */
...
/* ======================================================================
 * EXISTING DATA STRUCTURES (ENHANCED)
 * ====================================================================== */
...
/* ======================================================================
 * SECTION 2: FUNCTION PROTOTYPES (DECLARATIONS)
 * ====================================================================== */
...
void push_seat(PriorityQueue *pq, int seat_index, char *seat_id, int priority, time_t booking_time);
...
/* Route Graph Functions */
void init_route_graph(void);
...
/* Route Management Functions (Admin) */
void admin_route_management(void);
void create_new_route(void);
void add_waypoint_to_route(void);
void edit_waypoint(void);
void remove_waypoint(void);
void delete_route(void);
void view_route_details(void);
...
void toggle_user_status(void);
void manage_tickets(void);
void list_tickets(void);
void add_ticket_with_route(void);
...
void view_available_tickets_with_routes(void);
void book_ticket_with_route(void);
void view_my_bookings_with_details(void);
void cancel_booking(void);
void view_tickets(void);
...
/* ======================================================================
 * SECTION 4: TIME CONVERSION FUNCTIONS
 * ====================================================================== */
...
int parse_time_12h(char *time_str, int *hours, int *minutes) {
    ...
    // Try to parse as HH:MM AM/PM without space
    if(sscanf(time_str, "%d:%d%s", &h, &m, ampm) == 3) {
        *hours = h;
        *minutes = m;
        return 1;
...
/* ======================================================================
 * SECTION 7: PRIORITY QUEUE FUNCTIONS
 * ====================================================================== */
...
/* ======================================================================
 * SECTION 8: TREE FUNCTIONS FOR SEAT AVAILABILITY
 * ====================================================================== */
...
/* ======================================================================
 * SECTION 9: ROUTE GRAPH FUNCTIONS
 * ====================================================================== */
...
int add_waypoint(int route_id, char *name, float distance, float time) {
    ...
    // Auto-calculate price multiplier based on position
    if(route->waypoint_count == 0) {
        wp->price_multiplier = 1.0;  // Starting point
    } else {
        // Later points get slightly lower multiplier, except major cities
        float avg_distance = distance / route->waypoint_count;
        if(avg_distance > 50) {
            wp->price_multiplier = 0.95;  // Long distance discount
        } else if(avg_distance < 20) {
            wp->price_multiplier = 1.05;  // Short distance premium
        } else {
            wp->price_multiplier = 1.0;
        }

        // Special premium points
...
char* get_departure_time_at_waypoint(Route *route, int waypoint_idx, char *base_time) {
    ...
    // Parse base time (format: HH:MM AM/PM)
...
/* ======================================================================
 * SECTION 10: ROUTE MANAGEMENT FUNCTIONS (ADMIN)
 * ====================================================================== */
...
void add_waypoint_to_route() {
    ...
    // Auto-calculate price multiplier
    if(insert_after == 0) {
        wp->price_multiplier = 1.0;
    } else {
        float avg_distance = distance / route->waypoint_count;
...
void add_ticket_with_route() {
    ...
    // Get departure time for this specific ticket
    do {
    ...
    new_ticket.active = 1;  // Make sure ticket is active
...
void view_available_tickets_with_routes() {
    ...
    for(int i = 0; i < ticket_count; i++) {
        // Check if ticket is active AND has available seats
...
void book_ticket() {
    book_ticket_with_route();
}

void view_my_bookings() {
    view_my_bookings_with_details();
}
...
/* ======================================================================
 * SECTION 15: FILE HANDLING FUNCTIONS
 * ====================================================================== */
...
void load_routes() {
    ...
    if(!file) {
        init_route_graph();
        // Create a default route with auto price multipliers
